    ...
    <div className="mx-auto max-w-6xl px-5 py-16">
      ...
      {groupes.map((groupe) => (
        <section key={groupe.type} className="mt-14">
          ...
          <ul className="mt-5 grid gap-px border border-parchemin/12 bg-parchemin/12 sm:grid-cols-2 lg:grid-cols-3">
            {groupe.items.map((l) => (
              <li key={l.id} className="flex flex-col bg-noir p-6">
                <div className="flex items-baseline gap-3">
                  <span
                    ...
                    aria-hidden="true"
                  />
                  <span className="text-[0.7rem] tracking-[0.2em] text-acier uppercase">
                    {l.brand}
                  </span>
                </div>
                <p className="font-display mt-2 text-2xl text-ivoire">{l.reference}</p>
                <p className="mt-1 text-sm text-parchemin/70">{l.name}</p>
                {l.viscosity && (
                ...
                {l.usage && (
                  <p className="mt-3 text-sm leading-relaxed text-parchemin/60">{l.usage}</p>
                )}
              </li>
            ))}
          </ul>
        ...
        <ul className="carte mt-5 divide-y divide-parchemin/8">
          {tools.map((t) => (
            <li key={t.id} className="flex flex-wrap items-baseline gap-x-4 gap-y-1 px-5 py-4">
              <span className="text-ivoire">{t.name}</span>
              {t.category && (
                <span className="text-[0.68rem] tracking-[0.16em] text-laiton uppercase">
                  {t.category}
                </span>
              )}
              {t.description && (
                <span className="w-full text-sm text-acier sm:w-auto sm:flex-1 sm:text-right">
                  {t.description}
                </span>
              )}
            </li>
      ...
      <p className="mt-12 max-w-2xl text-xs leading-relaxed text-acier">
