...
(function(){
  ...
  let crmClients = [];
  let crmActivities = [];          // activités CRM (appels, RDV, tâches…) pour le cockpit / brief
  ...
  function afterLoad(){ buildIsinList(); updateStats(); renderSuggest(); renderPortfolio(); buildReportSelect(); buildOverviewClientSelect(); buildCockpitClientSelect(); renderOverview(); updateObsCount(); }
  ...
  function loadCrm(){
    if(!SB) return;
    // Fiches complètes (KYC, profil de risque, suivi) + activités : alimentent le cockpit client.
    return Promise.all([
  ...
  document.querySelectorAll('.dash-tab').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      if(activeTab==='cockpit') renderCockpit();
  ...
  function openAlloc(p){
    ...
    document.getElementById('al-save').addEventListener('click',()=>{
      ...
      savePosition(pos).then((persisted)=>{
        updateStats(); buildReportSelect(); buildOverviewClientSelect(); buildCockpitClientSelect();
  ...
  function removePosition(pos){
    ...
    updateStats(); buildReportSelect(); buildOverviewClientSelect(); buildCockpitClientSelect(); return Promise.resolve();
  }
...
<div class="footer"><p><strong>La Financière de Rochechouart</strong> · 58 rue de Monceau, 75008 Paris</p><p>Document confidentiel — Valorisations et niveaux indicatifs, non contractuels.</p></div>
  ...
  /* ===================================================================
     ONGLET — COCKPIT CLIENT / BRIEF PRÉ-RDV
     Vue 360 par client : encours total et par enveloppe, allocation
     actuelle vs cible, performance (coupons YTD / depuis origine),
     prochaines échéances, points de conformité, brief auto-généré.
     =================================================================== */
  ...
  // Détenteurs de produits structurés, rapprochés de leur fiche CRM (par id d'allocation, sinon par nom).
  ...
  // --- Points de conformité en attente (KYC, profil DDA/MIF, RGPD, LCB-FT, revue, actions) ---
  function complianceItems(crm, items){
    const out=[]; const add=(sev,label,detail)=>out.push({sev,label,detail:detail||''});
    if(!crm){ add('warn','Fiche CRM non reliée','Aucun contact CRM rattaché à ce détenteur — reliez l\'allocation à une fiche (Suivi produit › Allouer › Contact CRM).'); return out; }
    const miss=[];
    ...
    if(items.length && (!knowsStruct || lowKnow)) add('warn','Adéquation produits structurés à tracer','Le client détient des produits structurés — documenter connaissance/expérience et la justification du conseil.');
  ...
  function buildCockpitData(entry, items){
    ...
    // performance (coupons perçus, YTD) déduits des calendriers (cours réels validés/observés)
  ...
  function renderCockpit(){
    ...
    if(!name){ currentCockpit=null; host.innerHTML='<div class="sp-rep-empty"><p>Sélectionnez un client pour afficher son cockpit 360° et générer le brief pré-RDV. Les clients apparaissent ici dès qu\'un produit leur est alloué (onglet Suivi produit › Allouer).</p></div>'; return; }
    ...
    render(true);
    // charge les cours réels des produits vivants (cache partagé avec l'onglet Observations) puis recalcule
    ...
    Promise.all(need.map(isin=>{ const p=productsMap.get(isin); return loadProductSeries(p).then(dd=>obsDataCache.set(isin,dd)); }))
      .then(()=>{ if(activeTab!=='cockpit' || (ckSel&&ckSel.value!==name)) return; render(false); });
  }
  ...
  function cockpitDueRow(x){
    ...
    return `<div class="ov-risk__row ${sev}" data-isin="${esc(p.isin)}">
  ...
  function bindCockpit(d){
    ...
    document.querySelectorAll('#ck-body .ov-risk__row[data-isin]').forEach(el=>el.addEventListener('click',()=>gotoProduct(el.dataset.isin)));
  ...
  /* ---- Brief : lignes structurées (réutilisées pour l'affichage, le copier et l'impression) ---- */
